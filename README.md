...
- **进程** — system_server 视角的实时进程看板（pid / 状态 / oom_adj / RSS / 运行时长），支持导出快照
- **设置** — 深色模式、保活能力开关、优先级档位、常驻通知、备份恢复

支持**深色 / 浅色 / 跟随系统**三种主题，附带 4×1 桌面小部件。
...
**全新界面** —— 这一版彻底重做了 UI，从"能用就行"的工具界面变成完整的应用：

- **四页导航**：底部切换「主页 / 应用 / 进程 / 设置」，各页职责清晰。
- **主页仪表盘**：模块激活状态、Bento 概览卡片、**24 小时存活曲线**（自绘 Canvas 折线，可点应用图标下钻查看单个应用存活率）、守护事件时间线。
- **深色模式**：深色 / 浅色 / 跟随系统三档可选。
- **桌面小部件**：4×1 守护看板，不打开 App 也能看到保活状态。
- **能力自检页**：一键体检环境是否就绪（LSPosed 激活、作用域勾选、系统版本适配）。
...
- 版本号从 3.2.2 升到 3.3.0。versionCode 改为分段管理：debug 走 3000 段（时间戳命名），release 走 4000 段。
